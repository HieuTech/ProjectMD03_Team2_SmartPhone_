...
session.close();
    ...
    @Override
    public boolean addRole(Users user, Roles role) {
        try (Session session = sessionFactory.openSession()) {
            session.beginTransaction();
            boolean roleExists = user.getUserRoles().stream()
                    .anyMatch(userRole -> userRole.getRole().getRoleName().equals(role.getRoleName()));
    ...
    public boolean deleteRole(Integer userId, Integer roleId) {
        try (Session session = sessionFactory.openSession()) {
            session.beginTransaction();
            Query<UserRoles> query = session.createQuery("FROM UserRoles ur WHERE ur.users.id = :userId AND ur.role.id = :roleId", UserRoles.class);
            query.setParameter("userId", userId);
            query.setParameter("roleId", roleId);
            UserRoles userRole = query.uniqueResult();
